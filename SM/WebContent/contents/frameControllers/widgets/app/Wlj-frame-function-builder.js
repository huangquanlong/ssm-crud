...
Ext.extend(Wlj.frame.functions.app.Builder,Ext.util.Observable,{
	...
	getCodeFilePage : function(){
		if(parent.Wlj){
			...
			//===========================================================================================
			// wzy,2015-02-05,add:增加“直接加载一个JS文件”的处理逻辑
			}else if(parent.Wlj.TaskMgr.getTask("task_"+this.resId)){
				this.codeFile = parent.Wlj.TaskMgr.getTask("task_"+this.resId).action;
			//===========================================================================================
			}else{
		...
		if(this.buildFlow && !window._IM){
			Ext.ScriptLoader.loadScript({
				...
				finalCallback: function(response){
					...
					window._IM.currentStepResId = resId;
				}
			});
		}
	},
...
Ext.onReady(function(){
	...
	﻿Ext.getBody().on('click',function(e,h,o){﻿parent.Ext.menu.MenuMgr.hideAll();﻿});
});