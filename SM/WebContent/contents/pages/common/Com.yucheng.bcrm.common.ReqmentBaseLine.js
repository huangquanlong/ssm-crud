 ...
 */
Com.yucheng.bcrm.common.ReqmentBaseLine = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
		Com.yucheng.bcrm.common.ReqmentBaseLine.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
    ...
    onTrigger2Click : function(){
		...
		Ext.Ajax.request({//需求目录树数据加载
			url :basepath + '/ReqmentContetTreeInfoAction.json' ,
			method:'GET',
...
//					}
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    ...
    	    {header : '项目ID',dataIndex : 'PROJ_ID',width : 80,sortable : true,hidden:true},
    	    {header : '项目标识',dataIndex : 'PROJ_NO',width : 80,sortable : true,hidden:true},
    	    {header : '需求目录ID',dataIndex : 'REQ_DIR_ID',width : 150,sortable : true,hidden:true},
    	    {header : '项目名称',dataIndex : 'PROJ_NAME',width : 150,sortable : true},
    	    {header : '需求名称',dataIndex : 'REQ_NAME',width : 140,sortable : true},
    	    {header : '需求标识',dataIndex : 'REQ_NO',width : 100,sortable : true},
    	    {header : '需求目录',dataIndex : 'REQ_DIR_NAME',width : 120,sortable : true},
    	    {header : '需求基线',dataIndex : 'REQ_BASELINE',width : 80},
    	    ...
    	    {header : '需求描述',dataIndex : 'REQ_DESC',width : 140,hidden : true}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/ReqmentBaseInfoQueryAction.json'}),
    		reader: new Ext.data.JsonReader({
...
//    						checkedNodes = _this.oCustomerQueryGrid.getSelectionModel().selections.items;
						...
						for(var i=0;i<checkedNodes.length;i++){
							if(i==0){
								sName=checkedNodes[i].data.REQ_VER;
								oThisSearchField.hiddenField.setValue(sName);
							}else{
								sName=checkedNodes[i].data.REQ_VER;
								oThisSearchField.hiddenField.setValue(_this.hiddenField.value+','+checkedNodes[i].data.REQ_VER);
							}
						}
						oThisSearchField.setRawValue(sName);
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							oThisSearchField.projId=checkedNodes[0].data.PROJ_ID;
							...
							oThisSearchField.projNo=checkedNodes[0].data.PROJ_NO;
							oThisSearchField.reqName=checkedNodes[0].data.REQ_NAME;
							oThisSearchField.reqBaseLine=checkedNodes[0].data.REQ_BASELINE;
							oThisSearchField.reqDesc=checkedNodes[0].data.REQ_DESC;
...
Ext.reg('baseLine',Com.yucheng.bcrm.common.ReqmentBaseLine);