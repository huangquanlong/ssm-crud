...
Com.yucheng.bob.ExpButton = Ext.extend(Ext.Button,{
    ...
    handler:function(){
        var colMArray = [];
        var fieldArray=[];
    	if (this.colMArray != undefined) {
    		colMArray = this.colMArray;
		} else {
			colMArray= this.ownerCt.ownerCt.colModel.config;
		}
		if (this.fieldArray != undefined) {
			fieldArray = this.fieldArray;
		} else {
			fieldArray= this.ownerCt.ownerCt.store.fields.items;
		}
        ...
        if(typeof this.formPanel == 'string'){
            if(this.formPanel != ''){
                tmpFormPanel = Ext.getCmp(this.formPanel);
            }
        }else {
        ...
        if(tmpFormPanel!=undefined && tmpFormPanel.getXType()=='form'){
        	...
        	if (typeof this.beforeExport === 'function'){
        		var tempExParams = this.beforeExport();
        		if(tempExParams === false){
					return false;
				}
        		if (tempExParams && typeof tempExParams === 'object'){
        			this.exParams = tempExParams;
        		}
        	}
        	if(this.exParams ===false){
        		Ext.Msg.alert('提示','导出参数配置存在问题！');
        		return;
        	}
        }
        ...
        Ext.Ajax.request({
            ...
            success:function(a,b){
        		if (backgroundExport){
        			...
        			Com.yucheng.bob.showProgressBar(1,0,'','正在导出，请稍后...','导出文件');
                            ...
                            success:function(a){
                                if(a.status == '200' || a.status=='201'){
                                    ...
                                    if(res.json.filename){
                                    	Com.yucheng.bob.showSuccessWin(res.json.expRecNum,basepath+'/FileDownload?filename='+res.json.filename);
                                        if(refreshHandler != 0){
                                    	...
                                    	if ((typeof res.json.expRecNum) != 'undefined' && (typeof res.json.total) != 'undefined') {
                                    		Com.yucheng.bob.showProgressBar(res.json.total,res.json.expRecNum,'',getMsg(res.json.expRecNum,res.json.total),progressBarTitle);
                                    	} else {
                                    		 progressBar.hide();
...
Com.yucheng.bob.showProgressBar=function(count,curnum,bartext,msg,title) {
	importState = false;
...
Com.yucheng.bob.showSuccessWin=function(curRecordNum,filePath) {
	importState = true;
	progressWin.removeAll();
	...
	progressWin.add(new Ext.Panel({
		...
		html: '<img src="'+basepath+'/contents/images/UltraMix55.gif" />',
		buttonAlign: 'center',
		buttons: [
			new Ext.Button({
			  text:'下载',
