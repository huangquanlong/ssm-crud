...
Com.yucheng.crm.security.SecurityBooter.prototype.mainMask = new Ext.LoadMask(Ext.getBody(), {msg:SECUTILS.waitMsg});
...
Com.yucheng.crm.security.SecurityBooter.prototype.beforeHide = function() {
	var _this = this;
	//20141026,进入系统强制修改密码策略,必须修改后才能操作
	if(__updatePwdStat != '1'){
		Ext.Msg.alert('提示','您必须修改密码才能访问系统！');
		return false;
	}
	/**初始化权限是否已完成和配置判断*/
...
//修改密码页面内容URL
Com.yucheng.crm.security.SecUnit.prototype.pswChangeUrl = basepath + '/contents/pages/wlj/systemManager/changePassWord.jsp';
...
Ext.extend(Com.yucheng.crm.security.SecListUnit,Com.yucheng.crm.security.SecUnit,{
		...
		if (_this.getRoleInfoStore() == null) {
			_this.setRoleInfoStore(new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
					...
					method:'GET',
					success:function(){
						_this.getSecMainWin().mainMask.hide();
					}
				}),
	...
	rowDBClick : function(selModel) {
		var _this = this;
		//_this.getSecMainWin().mainMask.show();
		/**判断是否有选中行*/
