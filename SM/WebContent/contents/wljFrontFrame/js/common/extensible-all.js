		...
		.extend(
				...
				Ext.XTemplate,
				{
					...
					applyTemplate : function(n) {
						Ext.apply(this, n);
						var k = 0, u = "", f = true, v = false, h = false,ifDay = false,date = '',hasDay = false, t = false, a = false, m = false, e = n.weekendCls, p = n.prevMonthCls, s = n.nextMonthCls, b = n.todayCls, g = [ [] ], r = new Date()
								.clearTime(), l = this.viewStart.clone(), c = this.startDate
								.getMonth() ;
						date = n.startDate.format('Ymd');
										...
										.push({
											...
											titleCls :"ext-cal-dtitle "
													...
													+ (a ? " ext-cal-dtitle-next"
															: ""),
											cellCls : (ifDay?"ext-cal-day-chose ":"ext-cal-day ")+
													+ (v ? " " + b : "")
					...
					getTodayText : function() {
						var b = Ext.ensible.Date.use24HourTime ? "G:i "
								...
								+ this.id
								+ '-clock" class="ext-cal-dtitle-time">'
...
Ext.ensible.cal.DayViewDropZone = Ext
		.extend(
				Ext.ensible.cal.DropZone,
				{
					...
					dateRangeFormat : "{0}-{1}",
					dateFormat : "n/j",
					onNodeOver : function(c, o, m, i) {
						var b, p = this.createText, h = Ext.ensible.Date.use24HourTime ? "G:i"
								: "g:ia";
						if (i.type == "caldrag") {
							...
							var s = Ext.ensible.Date.copyTime(c.date,
									this.dragCreateDt);
							...
							var r = Ext.get(i.ddel), q = r.parent().parent(), k = r
									.getBox();
							k.width = q.getWidth();
							if (i.type == "eventdrag") {
							...
							if (i.type == "eventresize") {
								...
								var s = Ext.ensible.Date.copyTime(c.date,
										this.resizeDt), a = Ext.ensible.Date
										.min(i.eventStart, s), g = Ext.ensible.Date
										.max(i.eventStart, s);
								i.resizeDates = {
									StartDate : a,
									EndDate : g
						...
						i.proxy.updateMsg(String.format(p, b));
						return this.dropAllowed
					},
					shim : function(b, a) {
...
Ext.ensible.cal.WeekEventRenderer = function() {
	...
	return {
		render : function(n) {
			var g = 0, b = n.eventGrid, l = n.viewStart.clone(), m = n.tpl, r = n.maxEventsPerDay != undefined ? n.maxEventsPerDay
					: 999, t = n.weekCount < 1 ? 6 : n.weekCount, p = n.weekCount == 1 ? n.dayCount
					: 7;
			for (; g < t; g++) {
				var f, y = 0, c = b[g];
				var A = l.clone();
				...
				for (; y < p; y++) {
					if (c && c[y]) {
						var x = emptyCells = skipped = 0, s = c[y], e = s.length, k;
						for (; x < e; x++) {
							if (!s[x]) {
								emptyCells++;
								continue
							...
							if (!k.isSpan || k.isSpanStart) {
								...
								v._weekIndex = g;
								v._renderAsAllDay = v[Ext.ensible.cal.EventMappings.IsAllDay.name]
										|| k.isSpanStart;
								v.spanLeft = v[Ext.ensible.cal.EventMappings.StartDate.name]
										.getTime() < A.getTime();
								v.spanRight = v[Ext.ensible.cal.EventMappings.EndDate.name]
										.getTime() > h.getTime();
								v.spanCls = (v.spanLeft ? (v.spanRight ? "ext-cal-ev-spanboth"
										: "ext-cal-ev-spanleft")
										: (v.spanRight ? "ext-cal-ev-spanright"
												: ""));
								var f = a(n.id, g, x), q = {
									tag : "td",
									cls : "ext-cal-ev",
									cn : m.apply(n.templateDataFn(v))
										...
										.diffDays(
												l,
												v[Ext.ensible.cal.EventMappings.EndDate.name]) + 1, u = Math
										.min(i, p - y);
								if (u > 1) {
									q.colspan = u
								}
				...
				Ext.BoxComponent,
				{
					...
					forceSize : function() {
						if (this.el && this.el.child) {
							var e = this.el.child(".ext-cal-hd-ct"), b = this.el
									.child(".ext-cal-body-ct");
							if (b == null || e == null) {
								return
							}
							var a = e.getHeight(), c = this.el.parent()
									.getSize();
					...
					prepareEventGrid : function(c, b, g) {
						var f = 0, e = this.viewStart.clone(), a = this.maxEventsPerDay ? this.maxEventsPerDay
								: 999;
						c
								.each(
										function(i) {
											...
											if (Ext.ensible.Date.diffDays(
													i.data[k.StartDate.name],
													i.data[k.EndDate.name]) > 0) {
												var h = Ext.ensible.Date
														.diffDays(
																Ext.ensible.Date
																		.max(
																				this.viewStart,
																...
																Ext.ensible.Date
																		.min(
																				this.viewEnd,
																				i.data[k.EndDate.name])) + 1;
												this
														.prepareEventGridSpans(
																i,
																this.eventGrid,
																b, g, h);
												this.prepareEventGridSpans(i,
														this.allDayGrid, b, g,
														h, true)
											} else {
												f = this
														.findEmptyRowIndex(b, g);
												this.eventGrid[b][g] = this.eventGrid[b][g]
														|| [];
												this.eventGrid[b][g][f] = i;
												if (i.data[k.IsAllDay.name]) {
													f = this.findEmptyRowIndex(
															b, g, true);
											...
											if (this.evtMaxCount[b] < this.eventGrid[b][g].length) {
												this.evtMaxCount[b] = Math
														.min(
																a + 1,
																this.eventGrid[b][g].length)
											}
											return true
										}, this)
					},
				...
				Ext.ensible.cal.CalendarView,
				{
					...
					getEventHeight : function() {
						if (!this.eventHeight) {
							...
							if (a) {
								this.eventHeight = a.parent("tr").getHeight()
							} else {
								return 16
							}
						}
		...
		.extend(
				Ext.Panel,
				{
					...
					setActiveView : function(c) {
						var b = this.layout, a = this.getTopToolbar();
						b.setActiveItem(c);
					...
					updateNavState : function() {
						if (this.showNavBar !== false) {
							var b = this.layout.activeItem, c = b.id
									.split(this.id + "-")[1];
							if (this.showNavToday) {
								Ext.getCmp(this.id + "-tb-today").setDisabled(
										b.isToday())
							}
